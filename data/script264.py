...
# # Fixing Typos with Google 

# There are a lot of misspellings in the "search term" field ( as it is normal for inputs in a search engine).
# In this dataset I found that about 13% of "search term" has some typos.
# 
# spell_check is a simple function that uses google to remove typos.
# unfortunately it does not work on kaggle (it sends requests to google)
# 
...
# And this is the resulting dictionary for the typos searches
...
for search in searches:
	speel_check_search= spell_check_dict[search]
	print (search+"->" + speel_check_search)
