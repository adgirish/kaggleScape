...
# Fitting a logistic regression to Scirpus' [genetic features](https://www.kaggle.com/scirpus/big-gp).  The source of those features is still mysterious, since AFAIK Scirpus has not released his program yet.

# In[1]:


C_REGULARIZER = 3     # To regularize logistic fits: larger values mean less regularization
N_FOLDS = 5           # Used for both mean projection and logistic fits
SHOW_COEFS = False    # In case you're interested
...
def apply_model(data, test_data, preproc, state):
    LR = LogisticRegression(
        C=C_REGULARIZER, 
        fit_intercept=False  # Constant is already a feature
    )
    print( 'Processing data...' )
    K = N_FOLDS
    kf = KFold(n_splits = K, random_state = 1, shuffle = True)
    train_df = preproc(data)
    y_valid_pred = 0*data.target
    test_df = preproc(test_data)
    y_test = 0
    for i, (train_index, test_index) in enumerate(kf.split(data)):
        print('\nFold:',i)

        # Create data for this fold
        y, y_val = data.target.iloc[train_index].copy(), data.target.iloc[test_index].copy()
        X, X_val = train_df.iloc[train_index,:].copy(), train_df.iloc[test_index,:].copy()
        X_test = test_df.copy()

        model = LR.fit(X, y)
        del X, y
        if SHOW_COEFS:
            coefs = pd.Series( data=np.reshape(LR.coef_,-1), index=train_df.columns.values )
            print( 'Coefficients:', coefs )
        y_pred = model.predict_proba(X_val)[:,1]
        print(  'Gini score: ',  eval_gini( y_val, y_pred )  )
        y_valid_pred.iloc[test_index] = y_pred
        del X_val, y_val
        y_test += model.predict_proba(X_test)[:,1]  # but we should be using logistic average
    y_test /= K
    print(  'Gini score: ',  eval_gini( data.target, y_valid_pred  )  )
    return(  y_valid_pred, y_test  )
...
def GPI(data):
    v = get_GP1_features(data)
    return Outputs(v.sum(axis=1))

def GPII(data):
    v = get_GP2_features(data)
    return Outputs(v.sum(axis=1))
...
#    v[v>almost_one] = almost_one
#    v[v<almost_zero] = almost_zero
#    v = np.log(v/(1-v))
    return v
...
def GetData(strdirectory):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(range(train.shape[0]),train.target)):
        ...
        if(blindloodata is None):
            ...
            blinddataids = blindids.copy()
        else:
            blindloodata = pd.concat([blindloodata,blindtrain])
            blinddataids = pd.concat([blinddataids,blindids])
    ...
    return train, test, blinddataids
...
print('Started')
strdirectory = '../input/'
gptrain, gptest, ids = GetData(strdirectory)
...
print('Model 1')
(valid1, test1) = apply_model( gptrain, gptest, get_GP1_features, 1 )
gc.collect()
...
print('Model 2')
(valid2, test2) = apply_model( gptrain, gptest, get_GP2_features, 2 )
y_train = gptrain.target.copy()
...
print(   'Gini score for blended model: ',  eval_gini( y_train, (valid1+valid2)/2 )   )


# Warning:  There's a lot of leakage here, so don't take the validation score at face value.

# In[ ]:


print('Combine and save predictions')
basic = pd.read_csv(strdirectory+'sample_submission.csv')
test0 = ( test1 + test2 )/2
basic.target = test0.ravel()
basic.to_csv('gpx.csv',index=None,float_format='%.6f')


# (Not sure how to judge the right amount of regularization, when I don't trust the CV, but I guess I'll play it by ear.)

# Version notes:<br>
# 1. Baseline C=0.5,  CV=0.295<br>
# 2. Try C=0.7, CV=0.297<br>
# 3. no change<br>
# 4. Try C=0.8, CV=0.297<br>
# 5. Corrected typo.
# 6. Try C=1.0, CV=.298, LB=0.283 (sorts slightly higher than v.4)
# 7. Corrected typo.
