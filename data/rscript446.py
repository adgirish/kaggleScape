...
class KeplerMapper(object):
  ...
  def fit_transform(self, X, projection="sum", scaler=preprocessing.MinMaxScaler()):
    ...
    # Fix class detection for Python 3
    ...
    if isinstance(projection, str):
      ...
      if projection == "dist_median": # Distance of x to mean of X
        X_median = np.median(X, axis=0) 
        X = np.sum(np.sqrt((X - X_median)**2), axis=1).reshape((X.shape[0],1))
    ...
    for e, k in enumerate(complex["nodes"]):
      ...
      if custom_tooltips is not None:
        ...
        if color_function == "average_signal_cluster":
          ...
          json_s["nodes"].append({"name": str(k), "tooltip": tooltip_s, "group": 3 + 5 * int(np.log(len(complex["nodes"][k]))), "color": str(tooltip_i)})
        else:
          json_s["nodes"].append({"name": str(k), "tooltip": tooltip_s, "group": 3 + 5 * int(np.log(len(complex["nodes"][k]))), "color": str(k.split("_")[0])})
      else:
...
if __name__ == "__main__":
  ...
  # Fit to and transform the data, using 'sum(row)' as function
  ...
  complex = mapper.map(projected_X=projected_data, inverse_X=data, 
      clusterer=cluster.DBSCAN(eps=0.6, min_samples=6), 
      nr_cubes=30, overlap_perc=0.8)

  # Visualize it
  mapper.visualize(complex, path_html="output.html", 
      title="IRIS clustered on sum(row)", custom_tooltips=labels,
      width_html=900, height_html=600)

  # Initialize for 2nd visualization
  mapper = KeplerMapper(verbose=2)

  # Fit to and transform the data, using 'distance_to_median(row)' as function
  projected_data = mapper.fit_transform(data, projection="dist_median")

  # Create dictionary called 'complex' with nodes, edges and meta-information
  complex = mapper.map(projected_X=projected_data, inverse_X=data, 
      clusterer=cluster.DBSCAN(eps=0.9, min_samples=2), 
      nr_cubes=30, overlap_perc=0.9)

  # Visualize it
  mapper.visualize(complex, path_html="output2.html", 
      title="IRIS clustered on distance to median sample", custom_tooltips=labels)